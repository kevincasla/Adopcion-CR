        ...
        <?php
         if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== false) {
            echo '<div class="adopt-button" style="display: initial";>
               <a href="agregar_Perro.php" style="color: white;text-decoration-line: none;" ;="">Publicar mascota</a>
           </div>';
           }
        ?>
        <div class="dog-grid">
            ...
            $sql = 'SELECT ID_Perro, Nombre, Genero, Tamano, Numero_Contacto, Edad, Necesidades_Especiales, Comportamiento, Imagen FROM TAB_PERROS';
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '    </div>';
                    if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== false) {
                        echo '    <a href="adoptar_perros_foro.php?ID_Perro='.$row['ID_Perro'].'" class="adopt-button">¡Adóptame!</a>';
                        }
                    ...
                    if (isset($_SESSION['admin']) && $_SESSION['admin'] == 'si') {
                       ...
                       <div class="card-actions" style= "padding: inherit;">
                           <form action="editar_Perro.php" method="get" style="display:inline;">
                               <input id="id" type="hidden" name="id" value="' . $row["ID_Perro"] . '">
                               <button type="submit" class="btn btn-warning">Editar</button>
                           </form>
                           <form action="eliminar_Perro.php" method="get" style="display:inline;" onsubmit="return confirm(\'¿Estás seguro de que deseas eliminar este perro?\');">
                               <input type="hidden" id="id" name="id" value="' . $row["ID_Perro"] . '">
